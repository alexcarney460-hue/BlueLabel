...
const SOURCES = ['website', 'referral', 'social', 'email', 'paid', 'organic', 'direct', 'other'];
    ...
    <div className="p-6 lg:p-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <select
          ...
          className="px-3 py-2.5 bg-slate-800 border border-slate-700 rounded-xl text-sm text-white focus:outline-none focus:ring-2 focus:ring-sky-500"
        >
      ...
      {selected.size > 0 && (
        <div className="flex items-center gap-3 bg-slate-900 border border-slate-700 rounded-xl px-4 py-3">
          ...
          <button
            ...
            className="px-3 py-1.5 bg-red-600/20 border border-red-500/30 text-red-400 rounded-lg text-sm font-medium hover:bg-red-600/30 transition-colors"
          >
            Delete
          </button>
          <button
            onClick={() => setSelected(new Set())}
            className="text-sm text-slate-500 hover:text-white ml-auto"
          >
            Clear
          </button>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-800">
              {loading ? (
                ...
                <tr>
                  <td colSpan={9} className="px-4 py-12 text-center text-slate-500">No contacts found</td>
                </tr>
              ) : (
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select
                ...
                onChange={(e) => setAddForm({ ...addForm, lead_status: e.target.value })}
                className="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-sm text-white focus:outline-none focus:ring-2 focus:ring-sky-500"
              >
                {LEAD_STATUSES.map((s) => (
                  <option key={s} value={s}>{s.replace(/_/g, ' ')}</option>
            ...
            <div>
              ...
              <select
                value={addForm.lifecycle_stage}
                onChange={(e) => setAddForm({ ...addForm, lifecycle_stage: e.target.value })}
                className="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-sm text-white focus:outline-none focus:ring-2 focus:ring-sky-500"
              >
                {LIFECYCLE_STAGES.map((s) => (
                  <option key={s} value={s}>{s.replace(/_/g, ' ')}</option>
